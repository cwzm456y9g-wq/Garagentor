...
export class QuotesService {
  ...
  async create(dto: CreateQuoteDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const quoteNumber = await this.numbers.next('QUOTE', tx);
  ...
  async send(id: string) {
    ...
    const itemCount = await this.prisma.quoteItem.count({ where: { quoteId: quote.id } });
    if (itemCount === 0) {
  ...
  async remove(id: string) {
    ...
    if (quote.status !== QuoteStatus.ENTWURF) {
      return this.prisma.quote.update({ where: { id }, data: { status: QuoteStatus.STORNIERT } });
    }
