...
import { OperationMode, Prisma } from '@prisma/client';
...
export class DoorsService {
  ...
  async create(dto: CreateDoorDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const doorNumber = await this.numbers.next('DOOR', tx);
  ...
  async update(id: string, dto: UpdateDoorDto) {
    ...
    if (dto.siteId) {
      await this.customers.assertSiteBelongsToCustomer(
        dto.siteId,
        dto.customerId ?? door.customerId,
      );
    }
  ...
  async assertExists(id: string): Promise<void> {
    const count = await this.prisma.door.count({ where: { id } });
    if (count === 0) {
      throw new NotFoundException('Die Toranlage wurde nicht gefunden.');
    }
  }
