...
export class ServiceReportsService {
  ...
  async create(dto: CreateServiceReportDto) {
    return this.prisma.$transaction(async (tx) => {
      const reportNumber = await this.numbers.next('SERVICE_REPORT', tx);
  ...
  async complete(id: string, dto: CompleteServiceReportDto) {
    ...
    const materials = await this.prisma.serviceReportMaterial.findMany({
  ...
  async remove(id: string) {
    const report = await this.prisma.serviceReport.findUnique({ where: { id } });
    if (!report) {
      throw new NotFoundException('Der Servicebericht wurde nicht gefunden.');
    }
    ...
    await this.prisma.serviceReport.delete({ where: { id } });
  ...
  async createContract(dto: CreateMaintenanceContractDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const contractNumber = await this.numbers.next('MAINTENANCE_CONTRACT', tx);
  ...
  async expireContracts(): Promise<number> {
    const result = await this.prisma.maintenanceContract.updateMany({
