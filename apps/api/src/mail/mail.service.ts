...
import { currentUserId } from '../common/audit/request-context';
...
export class MailService {
  ...
  private get einstellungenAusUmgebung(): AppConfig['mail'] {
    return this.config.getOrThrow<AppConfig['mail']>('mail');
  }
  ...
  async senden(dto: SendMailDto) {
    ...
    const protokoll = await this.prisma.mailLog.create({
      data: {
  ...
  private async einstellungen(): Promise<MailEinstellungen> {
    const setting = await this.prisma.setting.findUnique({ where: { key: 'mail' } });
  ...
  private belegAlsPdf(art: MailDocumentType, id: string) {
    switch (art) {
      case 'ANGEBOT':
        return this.pdf.angebot(id);
      case 'RECHNUNG':
        return this.pdf.rechnung(id);
      case 'MAHNUNG':
        return this.pdf.mahnung(id);
      case 'SERVICEBERICHT':
        return this.pdf.servicebericht(id);
      case 'PRUEFPROTOKOLL':
        return this.pdf.pruefprotokoll(id);
    }
  }
  ...
  private async vermerkeVersand(art: MailDocumentType, id: string): Promise<void> {
    try {
      if (art === 'MAHNUNG') {
        await this.prisma.dunning.updateMany({
  ...
  private async belegDaten(art: MailDocumentType, id: string): Promise<BelegDaten> {
    const firma = await this.prisma.setting.findUnique({ where: { key: 'firma' } });
