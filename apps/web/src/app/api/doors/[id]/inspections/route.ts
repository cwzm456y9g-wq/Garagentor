...
import { startInspectionSchema } from '@/server/dienste/doors/dto/inspection.dto';
...
export const POST = geschuetzt<{ id: string }>(
  async (anfrage, { params }) => {
    return json(
      await inspectionsService.start(params.id, await rumpf(anfrage, startInspectionSchema)),
      201,
    );
  },
