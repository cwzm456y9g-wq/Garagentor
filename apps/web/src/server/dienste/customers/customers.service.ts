...
import { AddressType, type Prisma } from '@prisma/client';
...
export class CustomersService {
  ...
  async statistics(id: string) {
    ...
    const [revenue, open, lastInvoice, openQuotes] = await prisma.$transaction([
      ...
      prisma.quote.count({ where: { customerId: id, status: 'VERSENDET' } }),
    ]);
  ...
  async create(dto: CreateCustomerDto) {
    return prisma.$transaction(async (tx) => {
      const customerNumber = await numbers.next('CUSTOMER', tx);
  ...
  async removeAddress(customerId: string, addressId: string) {
    const result = await prisma.address.deleteMany({ where: { id: addressId, customerId } });
    if (result.count === 0) {
      throw new NotFoundException('Die Adresse wurde nicht gefunden.');
    }
  ...
  async removeContact(customerId: string, contactId: string) {
    const result = await prisma.contact.deleteMany({ where: { id: contactId, customerId } });
    if (result.count === 0) {
      throw new NotFoundException('Der Ansprechpartner wurde nicht gefunden.');
    }
  ...
  async addSite(customerId: string, dto: CreateSiteDto) {
    await this.assertExists(customerId);
    return prisma.site.create({ data: { ...dto, customerId } });
  }
  ...
  async updateSite(customerId: string, siteId: string, dto: UpdateSiteDto) {
    ...
    return prisma.site.update({ where: { id: siteId }, data: dto });
  }
  ...
  async removeSite(customerId: string, siteId: string) {
    ...
    if (site._count.doors > 0 || site._count.orders > 0) {
      ...
      return prisma.site.update({ where: { id: siteId }, data: { active: false } });
    }
