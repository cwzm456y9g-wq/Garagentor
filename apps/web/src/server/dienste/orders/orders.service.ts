...
import { InvoiceType, OrderStatus, type Prisma } from '@prisma/client';
...
export class OrdersService {
  ...
  async costs(id: string) {
    ...
    const [times, materials, invoiced] = await prisma.$transaction([
  ...
  async update(id: string, dto: UpdateOrderDto) {
    ...
    if (dto.siteId) {
      await customers.assertSiteBelongsToCustomer(dto.siteId, dto.customerId ?? order.customerId);
    }
  ...
  async remove(id: string) {
    ...
    if (order.status !== OrderStatus.ANGELEGT) {
      return prisma.order.update({ where: { id }, data: { status: OrderStatus.STORNIERT } });
    }
