...
import { AbsenceStatus, AbsenceType, type Prisma, Role } from '@prisma/client';
...
export class HrService {
  ...
  async create(dto: CreateEmployeeDto) {
    return prisma.$transaction(async (tx) => {
      const employeeNumber = await numbers.next('EMPLOYEE', tx);
  ...
  async updateQualification(employeeId: string, id: string, dto: UpdateQualificationDto) {
    const qualification = await prisma.qualification.findFirst({ where: { id, employeeId } });
    if (!qualification) {
  ...
  async removeQualification(employeeId: string, id: string) {
    const result = await prisma.qualification.deleteMany({ where: { id, employeeId } });
    if (result.count === 0) {
      throw new NotFoundException('Die Qualifikation wurde nicht gefunden.');
    }
    return { deleted: true, id };
