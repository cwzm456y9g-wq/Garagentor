...
import { articles } from '../inventory/articles.service';
import { doors } from '../doors/doors.service';
...
export class ReportsService {
  ...
  async dashboard(): Promise<DashboardSummary> {
    ...
    const [
      ...
      doors.inspectionsDue(),
      prisma.defect.count({ where: { status: { in: ['OFFEN', 'IN_BEARBEITUNG'] } } }),
      prisma.appointment.count({
        where: { start: { lte: dayEnd }, end: { gte: dayStart }, status: { not: 'ABGESAGT' } },
      }),
      articles.belowMinStock(),
      articles.stockValue(),
    ]);
