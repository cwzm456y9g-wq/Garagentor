...
export class SettingsService {
  ...
  async upsert(key: string, dto: UpsertSettingDto) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      await audit.record(tx, 'EINSTELLUNG_GEAENDERT', {
        entityType: 'SETTING',
        entityId: key,
        label: key,
      });
  ...
  async findPresets(settingKey: string) {
    return prisma.settingPreset.findMany({
      where: { settingKey },
  ...
  async removePreset(settingKey: string, id: string) {
    const vorlage = await this.findPreset(settingKey, id);
    await prisma.settingPreset.delete({ where: { id: vorlage.id } });
    return { deleted: true, id: vorlage.id };
  }

  private async findPreset(settingKey: string, id: string) {
    const vorlage = await prisma.settingPreset.findUnique({ where: { id } });
  ...
  async findNumberRanges() {
    const ranges = await prisma.numberRange.findMany({ orderBy: { entity: 'asc' } });
  ...
  async updateNumberRange(entity: string, dto: UpdateNumberRangeDto) {
    ...
    const existing = await prisma.numberRange.findUnique({ where: { entity } });
    ...
    return prisma.numberRange.upsert({
      where: { entity },
      update: {
  ...
  async previewNumber(entity: string) {
    ...
    return { entity, naechsteNummer: await numbers.preview(known.entity) };
  }
}
