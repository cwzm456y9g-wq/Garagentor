...
import { ConflictException, Logger, NotFoundException } from '@/server/nest-ersatz';
import {
  addDays,
  calculateInterest,
...
export class DunningService {
  ...
  async run(reference = new Date()) {
    ...
    for (const candidate of candidates) {
      try {
        const dunning = await prisma.$transaction(async (tx) => {
  ...
  async send(id: string) {
    const dunning = await prisma.dunning.findUnique({ where: { id } });
    if (!dunning) {
  ...
  private async dunningSetting(): Promise<{
    ...
    const setting = await prisma.setting.findUnique({ where: { key: 'mahnwesen' } });
